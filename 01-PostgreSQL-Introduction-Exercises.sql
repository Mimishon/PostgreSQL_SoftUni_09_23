...
*** Note, the database can only be executed by the database owner. It cannot be executed while you are connected to the target database. You can connect to "postgres" or any other database to issue this command. Also, if anyone else is connected to the target database, this command will fail unless you use the FORCE option.